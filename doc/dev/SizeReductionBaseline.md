...
`make bench` passes `--bench` to the test scripts. For `test-js-libs.sh`, that compares the generated table and SVG with the documented baseline. A matching benchmark stays quiet. A mismatch prints the generated benchmark table and fails so that the results can be reviewed.
